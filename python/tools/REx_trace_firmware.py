...
class TraceMgmt:
    ...
    def run_menu(self):
        ...
        while True:
            if len(trace_data):
                click.secho('Current trace: Device %s | Start timestamp %f: ' % (
                device_name, trace_data[0]['status']['timestamp']), fg="green", bold=True)
            else:
    ...
    def load_trace_from_device(self):
        device_names=['pimu','wacc','right_wheel','left_wheel','lift','arm']
        print(Style.BRIGHT + '############### Devices ################' + Style.RESET_ALL)
        for i in range(len(device_names)):
            print('%d: %s' % (i, device_names[i]))
        print('')
        device_id = self.get_int([0, 5], 'device ID')
        print('')
        trace_data = []
        device_name = device_names[device_id]
        if device_names[device_id] == 'pimu':
            trace_data = robot.pimu.read_trace()
        if device_names[device_id] == 'wacc':
            trace_data = robot.wacc.read_trace()
        if device_names[device_id] == 'right_wheel':
            trace_data = robot.base.right_wheel.read_trace()
        if device_names[device_id] == 'left_wheel':
            trace_data = robot.base.left_wheel.read_trace()
        if device_names[device_id] == 'lift':
            trace_data = robot.lift.motor.read_trace()
        if device_names[device_id] == 'arm':
            trace_data = robot.arm.motor.read_trace()
        if len(trace_data)==0:
