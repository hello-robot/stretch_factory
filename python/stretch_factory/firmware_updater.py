...
class FirmwareUpdater():
    ...
    def __create_arduino_config_file(self,user_path=None):
        if user_path==None:
    ...
    def __check_arduino_cli_install(self):
        target_version = target_version = b'0.18.3' #b'0.23.0'
        version = 'None'
    ...
    def do_update_to(self, no_prompts=False):
        # Return True if system was upgraded
        # Return False if system was not upgraded / error happened
        ...
        print('')
        print('')
        return self.do_update(no_prompts)

    def do_update_to_path(self, path_name, no_prompts=False):
        ...
        #Create new arduino-cli.yaml with correct user path
        self.__create_arduino_config_file(user_path=path_name)
        ...
        return self.do_update(no_prompts,repo_path=repo_path)

    def do_update_to_branch(self, no_prompts=False):
        # Return True if system was upgraded
        # Return False if system was not upgraded / error happened
            ...
            if self.use_device[device_name]:
                if not self.fw_installed.is_device_valid(device_name):  # Device may not have come back on bus
                    ...
                    if v_curr == self.target[device_name]:
                        click.secho(
                            '%s | %s ' % (device_name.upper().ljust(25), 'Installed firmware matches target'.ljust(40)),
                            fg="green")
                    else:
                        click.secho('%s | %s ' % (device_name.upper().ljust(25), 'Firmware update failure!!'.ljust(40)),
                                    fg="red", bold=True)
                        n_failure = n_failure + 1
        if n_failure != 0:
            click.secho('#' * 110, fg="red", bold=True)
            click.secho(
                'Firmware update reported %d failures.\nTo remedy failures power down and the power up the robot and try again.' % n_failure,
                fg="red", bold=True)
            return False
        return True
    ...
    def get_firmware_version_from_path(self, sketch_name, path):
        ...
        try:
            ...
            click.secho('Invalid path provided. Path should should have sketch directories under it', fg="red",
                        bold=True)
            return None
        lines = f.readlines()
        for l in lines:
    ...
    def get_port_name(self, device_name):
        try:
            port_name = Popen("ls -l /dev/" + device_name, shell=True, bufsize=64, stdin=PIPE, stdout=PIPE,
    ...
    def flash_firmware_update(self, device_name, tag, repo_path=None):
        ...
        if port_name is not None and sketch_name is not None:
            ...
            c = Popen(compile_command, shell=True, bufsize=64, stdin=PIPE, stdout=PIPE,
                      close_fds=True).stdout.read().strip()
            cc = c.split(b'\n')
            success = cc[-1][-6:] == b'bytes.' #Note works for v18.03 but not later versions necesarrily
            if verbose:
            ...
            u = Popen(upload_command, shell=True, bufsize=64, stdin=PIPE, stdout=PIPE,
                      close_fds=True).stdout.read().strip()
            uu = u.split(b'\n')
            if verbose:
                print(upload_command)
            ...
            if not success:
                print('Firmware failed to upload to %s' % (port_name))
            else:
                print('Success in firmware upload.')
                time.sleep(2.0)  # Give time to get back on bus
                s = StretchDeviceMgmt([device_name])
                s.reset_all()
